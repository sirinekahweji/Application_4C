<template>
  <div>
    ...
    gap: 30px; flex-grow: 1;">
          ...
          <div class="tableau">
            <div class="con">
              <h4>Liste des Participants Event :</h4>
              <div class="ligne">
          ...
          <div class="tableau">
            <div class="con">
              <h4>Liste des Presences Event :</h4>
            </div>
            <table class="table" style="margin-left:5px">
              <thead>
                <tr>
...
export default {
  ...
  methods: {
    ...
    deleteEvenement() {
      EvenementService.deleteEvenement(this.idEvenement)
      .then(() => {
          Swal.fire(
            'Deleted!',
            'Your Evenement has been deleted.',
            'success'
          ).then(() => {
        ...
        .catch(() => {
          Swal.fire(
            'Error!',
            'There was an error deleting your Evenement.',
          );
        });
    },
    GetParticipantsEvent() {
      EvenementService.GetParticipantsEvent(this.idEvenement)
        .then((res) => {
          if (res.data.length > 0) {
            res.data.forEach(participant => {
              EvenementService.GetUser(participant.user_id)
                .then((res) => {
            ...
            console.log("Aucun participant trouvé pour ce Evenement.");
          }
        })
        .catch((error) => {
          console.error('Une erreur s\'est produite lors de la récupération de participants event :', error);
        });
    },
    GetPresenceEvent() {
      EvenementService.GetPresenceEvent(this.idEvenement)
        .then((res) => {
          if (res.data.length > 0) {
            res.data.forEach(participant => {
              EvenementService.GetUser(participant.user_id)
                ...
                .catch((error) => {
                  console.error('Une erreur s\'est produite lors de la récupération de presence event :', error);
                });
            });
          } else {
            console.log("Aucun participant trouvé pour ce evenement.");
