  ...
  <div>
      ...
      <div class=" modal-dialog modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form @submit.prevent="updateAttestation">
              <div class="row">
                <!-- Partie gauche du formulaire -->
                <div class="col-md-6">
                  <div class="mb-3">
                    <label for="nom" class="form-label">Nom d'Attestation</label>
                    <input v-model="attestation.nom" type="text" class="form-control" id="nom" required>
                  </div>
                </div>
                ...
                <div class="col-md-6">
                  <div class="mb-3">
                    ...
                    <input type="text" v-model="attestation.type" class="form-control" id="lieu" required>
                  </div>
                </div>
              </div>
              <div>
                <label for="message" class="form-label">Description De Attestation</label>
                <textarea class="form-control" v-model="attestation.description" id="message" rows="3"></textarea>
              </div>
    ...
    <div class="container">
        ...
        <div class="buttons-right">
          ...
          <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal"
            @click="ouvrirFormulaireModification"><img :src="require('@/assets/participant.png')"
              style="width:20px ; height:20px">Modifier Attestation</button>
          ...
          <button type="button" class="btn btn-primary" @click="afficherContenu('userAttestation')"><img
      ...
      <div class="card-container">
        ...
        <div class="container" style="display: flex;" v-if="contenu === 'attestation'">
          <div class="col-md-6">
            <div class="mb-3 d-flex align-items-center">
              <label for="nom" class="form-label">Nom de Attestation</label>
              <span id="nomSpan" class="form-c ml-2">{{ attestation.nom }}</span>
            </div>
            <div class="mb-3 d-flex align-items-center">
              <label for="nom" class="form-label">Type</label>
              <span id="nomSpan" class="form-c ml-2">{{ attestation.type }}</span>
            </div>
            <div class="mb-3 d-flex align-items-center">
              <label for="nom" class="form-label">Description</label>
              <span id="nomSpan" class="form-c ml-2">{{ attestation.description }}</span>
            </div>
          ...
          <div class="col-md-6">
            <div class="photo-c ml-2" style=" width:500px;     height: 400px;">
          ...
          <div class="data">
            ...
            <h4 class="d-flex justify-content-between align-items-center">
              ...
              <div class="buttons-right ml-auto">
                ...
                <button v-else type="button" class="btn btn-primary" @click="AttribueAttestation"
        ...
        <div v-else style="display: flex;flex-direction: column;gap: 30px; flex-grow: 1;">
          <div class="data">
            ...
            <h4 class="d-flex justify-content-between align-items-center">
              <span>Utilisateurs avec attestation</span>
            </h4>
...
import AttestationService from '@/services/attestations';
...
export default {
  ...
  created() {
    this.idAttestation = this.$route.params.id;
    this.$watch('searchQuery', () => {
      this.recherche();
    });
    this.getDetailAttestation();
  },
  methods: {
    AttribueAttestation() {
      for (let i = 0; i < this.selectedProfiles.length; i++) {
        this.UserAttestation(this.selectedProfiles[i]);
      }
      this.selectedProfiles = [];
      this.getProfils();
    },
    getProfils() {
      userService.getUsersWithoutAttestation(this.idAttestation).then((res) => {
        this.profils = res.data;
        //console.log(res.data);
      })
    },
    getUsersAttestation(){
      userService.getUsersWithAttestation(this.idAttestation).then((res) => {
        this.profils = res.data;
        //console.log(res.data);
      })
    },
    ...
    getDetailAttestation() {
      AttestationService.getDetailAttestation(this.idAttestation)
        .then((res) => {
          this.attestation = res.data;
          this.imagepath = this.attestation.image
    ...
    updateAttestation() {
      AttestationService.updateAttestation(this.idAttestation, this.attestation);
    },
    UserAttestation(UserId) {
      AttestationService.UserAttestation(this.idAttestation, UserId)
    },


    afficherContenu(option) {
      this.contenu = option; // Change le contenu en fonction de l'option sélectionnée
      if(option=="userAttestation"){
        this.getUsersAttestation();
      }
