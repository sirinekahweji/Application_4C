            ...
            <div class="content">
                <div class="container">
                    ...
                    <div class="data">
                        ...
                        <h4>Liste des demandes d'accès</h4>
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    <tr v-for="(profil, index) in displayedItems" :key="index">
                                        ...
                                        <td v-if="demandesAcces[index] && demandesAcces[index].type !== null">{{ demandesAcces[index].type }}</td>
                                        <td>
                                            <router-link :to="'/profil/'+profil.id">
                                                <button class="action-button" type="button">
                                                    <i class="bi bi-eye"></i>
                                                </button>
                                            </router-link>
                                            <button class="action-button" type="button"
                                                @click="validerAcces(profil.id, this.demandesAcces[index].type, this.demandesAcces[index].id)">
                                                <i class="bi bi-check2"></i>
                                            </button>
                                            <button class="action-button" type="button"
                                                @click="confirmRefusAcces(this.demandesAcces[index].id)">
    ...
    methods: {
        ...
        recherche() {
            if (this.searchQuery === '') {
                ...
                this.profils = this.profils.filter(profil => {
                    ...
                    profil.prenom.toLowerCase().includes(this.searchQuery.toLowerCase());
                });
            }
        },
        AccepteTeacher(id, type) {
            console.log(id, type);
            userService.acceptUserAccess(id,{role_name:type})
            .then(() => {
                    alert('Accès accordé et rôle mis à jour avec succès!');
                })
                .catch(error => {
        ...
        getAcces() {
            demandesAccesService.index().then((res) => {
                ...
                const accessIds = this.demandesAcces.map(demande => demande.user_id);
        ...
        deleteDemande(id) {
            demandesAccesService.delete(id).then(() => {
                this.getProfils();
            })
        },
    },
