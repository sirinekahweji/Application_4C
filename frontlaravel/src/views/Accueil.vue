          ...
          <div class="posts" v-if="role.nomRole === 'administrateur' || role.nomRole === 'responsablemanager'">
            <div v-for="post in posts" :key="post.id" class="facebook-post">
                ...
                <div :id="'carouselExampleAutoplaying' + post.id" class="carousel slide" data-bs-ride="carousel">
                  <div class="carousel-inner">
                    ...
                    <div v-for="(image, index) in getFilteredImages(post.id)" :key="image.id"
                      ...
                      <img :src="getImageSource(image.image)" class="d-block w-100" alt="Image" />
                    </div>
                  </div>
              ...
              <div class="post-interactions">
                ...
                <button style="padding:14px;color:black" class="modal-trigger" @click="handleButtonClick(post.id)">comment</button>
              </div>
            </div>
          </div>
                  ...
                  <div v-for="(badge, index) in badges" :key="badge.id" class="badge-info">
                    <div class="to">
                      <div class="position-relative">
                        <img :src="getImageSource(badge.image)" alt="Badge Image" />
                        <span class="badge rounded-pill">
                          <img v-if="UserBadge[index]" :src="getImageSourcebagde(UserBadge[index])" alt="Owner Image" />
                        </span>
                      </div>
...
export default {
  ...
  computed: {},
  methods: {
    getImageSourcebagde(badge) {


      if (badge.photoProfile) {

        if (badge.photoProfile.startsWith('http')) {

          return badge.photoProfile;
        } else {
          return require(`@/assets/PhotosProfils/${badge.photoProfile}`);
        }
      } else {
    ...
    getUserById() {
      // Ensure that the loop doesn't exceed the length of the badges array
      const numberOfBadges = Math.min(this.badges.length, 6);

      for (let i = 0; i < numberOfBadges; i++) {
        if (this.badges[i] && this.badges[i].user_id) {
          UserService.getUser(this.badges[i].user_id)
            .then(res => {
          ...
          console.error(`Badge at index ${i} is undefined or does not have a user_id`);
        }
      }
    },
    getLastSixBadges() {
      BadgesService.getLastSixBadges()
        .then(res => {
          this.badges = res.data;
          this.getUserById();
        })
        .catch(error => {
          console.error("Error retrieving last six badges", error);
        });
    },
    getUserRole() {
