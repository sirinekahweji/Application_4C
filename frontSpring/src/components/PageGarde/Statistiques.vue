    ...
    methods: {
        ...
        countEtudiant() {
            EtudiantService.countEtudiants().then((res) => {
                this.statisticItems[3].target = res.data;
                this.animateCounters();
            })
        },

        countEnseignant() {
            EnseignantService.countEnseignants()
        .then((response) => {
          //console.log("ense:", response.data);
          this.statisticItems[4].target = response.data;
                this.animateCounters();        })
        
                .catch((error) => {
          console.log("Erreur lors de la récupération des rnseignnts :", error);
        });
    },
       ...
       /* countEnseignant() {
        ...
        countCertificat() {
            CertificatService.countCertif().then((res) => {
                this.statisticItems[1].target = res.data;
                this.animateCounters();
            })
        },
        countFormation4c() {
            Formation4cService.countFormations4c().then((res) => {
                this.statisticItems[0].target = res.data;
                this.animateCounters();
            })
        },
        countEvent() {
            EventService.count().then((res) => {
                this.statisticItems[2].target = res.data;
                this.animateCounters();
            })
        },
        animateCounters() {
